...
|--------|------|-------------|
| `GET` | `/dashboard/completion-stats` | Average completion time KPIs + company bar chart |

### `GET /dashboard/completion-stats`
...
**Metrics computed from cases with `dateDispatched` set:**

| Metric | Calculation |
|--------|-------------|
| Avg. time to first open | `firstOpenedAt − dateDispatched` |
| Avg. time to complete | `end − firstOpenedAt` (end = submit date or now) |
| Avg. total turnaround | `end − dateDispatched` |
| Expired cases | Counted at **10 days** |

Trends compare the selected period to the prior period of equal length.
...
```json
{
  ...
  "summary": {
    "avgTimeToFirstOpen": { "value": 24, "unit": "hours", "trend": -2, "trendUnit": "hours" },
    "avgTimeToComplete": { "value": 4, "unit": "days", "trend": -0.4, "trendUnit": "days" },
    "avgTotalTurnaround": { "value": 5.2, "unit": "days", "trend": 0.3, "trendUnit": "days" }
  },
  "overallAvgDays": 5.2,
  "byCompany": [
    {
...
**Case timing fields** (set automatically):

- `dateDispatched` — when secure link is sent (create or resend)
- `firstOpenedAt` — when recipient verifies/opens the link
...
| `dashboardService.getCompletionStats(period)` | `GET /dashboard/completion-stats?period=` |
...
| Dashboard overview | `GET /cases` (top metrics, status breakdown) |
| Dashboard completion time | `GET /dashboard/completion-stats` |
